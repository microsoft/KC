...
for K in 1 5; do
 for split in 1 2 3 4 5; do
    CUDA_VISIBLE_DEVICES=0 python proto/proto_emb.py \
        --test-path data/${dataset}/few_shot/0${split}/test.jsonl \
        --output-path output/${dataset}/0${split}/proto-5way${K}shot-paraphrase-distilroberta-base-v2-euc-wotrain-label \
        --encoder sentbert \
...
for K in 1 5; do
 for split in 1 2 3 4 5; do
   CUDA_VISIBLE_DEVICES=0  python proto/proto_emb.py \
     --test-path data/${dataset}/few_shot/0${split}/test.jsonl \
     --output-path output/${dataset}/0${split}/proto-5way${K}shot-nli-roberta-base-v2-euc-wotrain-label \
     --encoder sentbert \
...
for K in 1 5; do
 for split in 1 2 3 4 5; do
   CUDA_VISIBLE_DEVICES=0  python proto/proto_emb.py \
     --test-path data/${dataset}/few_shot/0${split}/test.jsonl \
     --output-path output/${dataset}/0${split}/proto-5way${K}shot-simcse-nli-euc-wotrain-label \
     --encoder bert \
...
for K in 1 5; do
 for split in 1 2 3 4 5; do
   CUDA_VISIBLE_DEVICES=0  python proto/proto_emb.py \
     --test-path data/${dataset}/few_shot/0${split}/test.jsonl \
     --output-path output/${dataset}/0${split}/proto-5way${K}shot-declutr-base-euc-wotrain-label \
     --encoder bertmean \
...
for K in 1 5; do
 for split in 1 2 3 4 5; do
   CUDA_VISIBLE_DEVICES=0  python proto/proto_emb.py \
     --test-path data/${dataset}/few_shot/0${split}/test.jsonl \
     --output-path output/${dataset}/0${split}/proto-5way${K}shot-sp-paraphrase-cos-wotrain-label \
     --encoder para \
