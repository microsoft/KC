...
class GrammarBasedState(State['GrammarBasedState']):
    ...
    def __init__(self,
                 ...
                 rnn_state: List[RnnStatelet],
                 grammar_state: List[GrammarStatelet],
                 sexpression_state: List[SExpressionState],
                 possible_actions: List[List[ProductionRule]],
        ...
        self.sexpression_state = sexpression_state
    ...
    def new_state_from_group_index(self,
        ...
        try:
            new_expression_state = self.sexpression_state[group_index].take_action(production_rule)
        except Exception as e:
            print("Error on new_state_from_group_index:\n")
            exec_info = sys.exc_info()
            traceback.print_exception(*exec_info)
            new_expression_state = self.sexpression_state[group_index]
            new_expression_state.early_stop = True
        ...
        return GrammarBasedState(batch_indices=[batch_index],
                                 ...
                                 sexpression_state=[new_expression_state],
    ...
    def get_valid_actions(self) -> List[Dict[str, Tuple[torch.Tensor, torch.Tensor, List[int]]]]:
        ...
        valid_actions = []
        for expression_state, grammar_state in zip(self.sexpression_state, self.grammar_state):
            actions = grammar_state.get_valid_actions()
            try:
                actions = expression_state.get_valid_actions(actions)
            except Exception as e:
                print("Error on get_valid_actions:\n")
                print(e)
                exec_info = sys.exc_info()
                traceback.print_exception(*exec_info)
                expression_state.early_stop = True
    ...
    def is_valid(self) -> bool:
        # if early stop, do not do anything
        if self.sexpression_state[0].early_stop:
            return False
        else:
            return True
    ...
    def combine_states(cls, states: Sequence['GrammarBasedState']) -> 'GrammarBasedState':
        ...
        sql_states = [sql_state for state in states for sql_state in state.sexpression_state]
