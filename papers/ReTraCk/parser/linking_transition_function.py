...
class LinkingTransitionFunction(BasicTransitionFunction):
    ...
    def __init__(
        ...
        self._use_attention_select = use_attention_select
    ...
    def _compute_action_probabilities(
        ...
        for group_index in range(group_size):
            ...
            if "linked" in instance_actions:
                linking_scores, linked_embeddings, linked_actions = instance_actions["linked"]
                ...
                if output_action_embeddings is not None:
                    output_action_embeddings = torch.cat(
                        [output_action_embeddings, linked_embeddings], dim=0
                    )
                else:
                    output_action_embeddings = linked_embeddings

                if self._mixture_feedforward is not None and embedded_action_logits is not None:
                    ...
                    mix1 = torch.log(torch.clamp(mixture_weight, min=1e-5, max=1 - 1e-5))
                    mix2 = torch.log(torch.clamp(1 - mixture_weight, min=1e-5, max=1 - 1e-5))
                    entity_action_probs = (
