...
def lisp_to_verify_sparql(lisp_program: str):
    clauses = []
    order_clauses = []
    entities = set()  # collect entites for filtering
    # identical_variables = {}   # key should be smaller than value, we will use small variable to replace large variable
...
def lisp_to_verify_sparql_xsl_only(lisp_program: str):
    clauses = []
    order_clauses = []
    entities = set()  # collect entites for filtering
    # identical_variables = {}   # key should be smaller than value, we will use small variable to replace large variable
...
if __name__ == '__main__':
    ...
    for item in train:
        sparql_query = lisp_to_sparql(item['s_expression'])  # simply call lisp_to_sparql
        # The resulted sparql query is executable against a Virtuoso server
